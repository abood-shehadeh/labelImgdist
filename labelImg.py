...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        load_classes = action('Load Classes', self.load_classes_dialog,
                              None, 'labels', 'Load Predefined Classes from file')
        ...
        add_actions(self.menus.file,
                    (open, open_dir, change_save_dir, open_annotation, 
                     copy_prev_bounding, self.menus.recentFiles, 
                     load_classes,  # <--- Added here
                     save, save_format, save_as, close, reset_all, 
                     delete_image, quit))
    ...
    def save_labels(self, annotation_file_path):
        ...
        def format_shape(s):
            return dict(label=s.label,
                        ...
                        # FIXED: Cast x and y to int() to prevent YOLO save crash
    ...
    def load_classes_dialog(self):
        path = os.path.dirname(ustr(self.file_path)) if self.file_path else '.'
        filename = QFileDialog.getOpenFileName(self, 'Choose Class File', path, "Text files (*.txt)")
        
        # Handle tuple return for different PyQt versions
        if isinstance(filename, (tuple, list)):
            filename = filename[0]

        if filename and os.path.isfile(filename):
            # Clear existing classes but keep the list object reference 
            # so the LabelDialog updates correctly
            ...
            # Feedback to user
            self.statusBar().showMessage(f'Loaded classes from {filename}')
            self.statusBar().show()
