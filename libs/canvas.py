...
class Canvas(QWidget):
    ...
    def mouseMoveEvent(self, ev):
        ...
        if self.drawing():
            ...
            if self.current:
                ...
                self.parent().window().label_coordinates.setText(
                        'Width: %d, Height: %d / X: %d; Y: %d' % (int(current_width), int(current_height), int(pos.x()), int(pos.y())))
                ...
                self.prev_point = pos
            self.update()  # CHANGED: repaint() -> update() to fix LAG
            return
        ...
        if Qt.RightButton & ev.buttons():
            if self.selected_shape_copy and self.prev_point:
                ...
                self.update() # CHANGED: repaint() -> update()
        ...
        if Qt.LeftButton & ev.buttons():
            if self.selected_vertex():
                ...
                self.scrollRequest.emit(int(delta_x), Qt.Horizontal) # Cast to int
                self.scrollRequest.emit(int(delta_y), Qt.Vertical)   # Cast to int
                self.update()
            return
    ...
    def paintEvent(self, event):
        ...
        if self.current is not None and len(self.line) == 2:
            ...
            p.setBrush(brush)
            # FIXED: Wrapped all float arguments in int() to prevent TypeError
            p.drawRect(int(left_top.x()), int(left_top.y()), int(rect_width), int(rect_height))

        if self.drawing() and not self.prev_point.isNull() and not self.out_of_pixmap(self.prev_point):
            p.setPen(QColor(0, 0, 0))
            # FIXED: Wrapped all float coordinates in int() to prevent TypeError
